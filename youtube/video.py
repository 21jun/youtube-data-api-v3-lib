...
class YouTubeVideo:
    ...
    def __init__(self, developer_key_index):
        ...
        self.youtube = build(YOUTUBE_API_SERVICE_NAME,
                             YOUTUBE_API_VERSION, developerKey=self.developer_key)
        self.db = DataBase()

    def fetch_all_games(self, condition=""):
    ...
    def fetch_video_ids(self, appid='', name='', qtype='appid'):
        ...
        video_ids = [{'appid': id[0], 'name': id[1], 'videoId': id[2]}
                     for id in video_ids]
        return video_ids
        ...
        for i, game in enumerate(games):
            ...
            for j, video in enumerate(video_ids):
                print(j, '/', len(video_ids),
                      video['name'], '|', video['appid'], '|', video['videoId'])
                self.get_video_info(
                    game['appid'], game['name'], video['videoId'], db_insert=db_insert)
        ...
        if db_insert:
            try:
                self.db.cur.execute(SQL.format(appid=appid, name=name, video_id=video_id, title=title, channel_id=channel_id,
                                               ...
                                               categoryId=category_id, tags=tags))
